...
class PerWaypointActionInProjV2(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, timesteps: torch.Tensor) -> torch.Tensor:
        ...
        timestep_feats = self.timestep_fourier_encoder(timesteps[..., -1]) # 前面填充了一个废弃维度，这里丢掉就是B，T
        timestep_feats = timestep_feats.repeat(1, T, 1) # 最后一个维度是timestep_feats的维度，也是action_feats
        x = torch.cat((action_feats, timestep_feats), dim=-1)
        return self.norm(self.encoder(x.flatten(0, 1)).reshape(B, T, -1)) # 只对最后一个dim维度过MLP
